...
import { Globe, Headphones, Flame, Radio, MapPin, Mail, Phone, Youtube, Facebook, Twitter, Heart, ChevronRight } from "lucide-react"
import { Link } from "react-router-dom"

const LEADERSHIP = [
  { name: "Pastor Emmanuel", role: "Senior Pastor & Founder", initials: "PE", gradient: "from-blue-600 to-cyan-500" },
  { name: "Pastor Grace", role: "Associate Pastor", initials: "PG", gradient: "from-indigo-600 to-blue-600" },
  { name: "Prophet Samuel", role: "Lead Prophet", initials: "PS", gradient: "from-blue-700 to-indigo-700" },
  { name: "Prophetess Ruth", role: "Prophetess", initials: "PR", gradient: "from-cyan-700 to-blue-700" },
  { name: "Deacon John", role: "Head Deacon", initials: "DJ", gradient: "from-slate-700 to-blue-800" },
  { name: "Sister Mary", role: "Women's Ministry", initials: "SM", gradient: "from-blue-800 to-indigo-800" },
]
...
const SOCIALS = [
  { icon: Youtube, label: "YouTube", handle: "@repentanceholiness", color: "text-red-400", bg: "bg-red-400/10 border-red-400/20" },
  { icon: Facebook, label: "Facebook", handle: "RepentanceAndHoliness", color: "text-blue-400", bg: "bg-blue-400/10 border-blue-400/20" },
  { icon: Twitter, label: "Twitter / X", handle: "@JILRadioKe", color: "text-cyan-400", bg: "bg-cyan-400/10 border-cyan-400/20" },
]
      ...
      <div className="space-y-5 pb-28 md:pb-10">
        ...
        >
          <div className="flex items-center gap-4">
            <div className="h-16 w-16 shrink-0 overflow-hidden rounded-2xl border border-white/10 shadow-lg">
              ...
              <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-blue-600 to-cyan-500 text-sm font-black text-white">JIL</div>
            </div>
            <div>
              <p className="text-[10px] font-black tracking-widest text-cyan-400 uppercase">✝ Est. 2010</p>
              <h1 className="text-xl font-black text-white leading-tight" style={{ fontFamily: "'Cinzel', serif" }}>
                Ministry of Repentance<br /><span className="text-cyan-400">& Holiness</span>
              </h1>
        ...
        <div className="rounded-2xl border border-white/5 bg-white/3 p-5">
          <div className="mb-3 flex items-center gap-2">
            <div className="h-6 w-6 flex items-center justify-center rounded-lg bg-amber-500/10 border border-amber-500/20">
              <Flame size={13} className="text-amber-400" />
            </div>
            <h2 className="text-sm font-black text-white">Our Mission</h2>
          </div>
          <p className="text-sm leading-relaxed text-slate-400">
            To preach the full Gospel of Jesus Christ with signs and wonders following — calling men & women to repentance, holiness, and a radical daily walk with the Holy Spirit. We proclaim that Jesus is the only way, truth, and life.
          </p>
        ...
        <Link to="/jesus-is-lord-radio">
          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
            className="relative overflow-hidden rounded-2xl border border-white/5 p-5 cursor-pointer"
            style={{ background: "linear-gradient(145deg, #060614, #0a0a1e, #0d1240)" }}
          >
            <div className="flex items-center justify-between">
              <div>
                <div className="mb-1 flex items-center gap-2">
                  <Radio size={15} className="text-cyan-400" />
                  <span className="text-xs font-black text-cyan-400">Jesus Is Lord Radio</span>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
            {LEADERSHIP.map((leader, i) => (
              ...
              >
                <div className={`flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br ${leader.gradient} text-sm font-black text-white shadow`}>
                  {leader.initials}
                </div>
                <div>
        ...
        <div className="rounded-2xl border border-white/5 bg-white/3 p-5 space-y-3">
          <h2 className="text-sm font-black text-white mb-4">Contact & Socials</h2>
          {[
            ...
            <div key={label} className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-white/5 border border-white/5">
          ...
          <div className="mt-4 grid gap-2">
            {SOCIALS.map(({ icon: Icon, label, handle, color, bg }) => (
              <button key={label} className={`flex items-center gap-3 rounded-xl border p-3 ${bg} hover:opacity-80 transition-opacity`}>
