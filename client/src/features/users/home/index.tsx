...
function LiveRadioCard() {
  ...
  return (
    <motion.div
      ...
      style={{
        background:
          "linear-gradient(135deg,var(--app-card) 0%,var(--app-surface) 100%)",
        borderColor: "var(--app-border)",
      }}
    >
      {/* Accent glow — only visible in dark */}
      <div
        className="pointer-events-none absolute -top-8 left-1/2 hidden h-24 w-24 -translate-x-1/2 rounded-full dark:block"
        style={{
          background:
            "radial-gradient(circle,rgba(0,140,255,.14) 0%,transparent 70%)",
        }}
      />
      ...
      <div
        className={`relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full shadow-lg transition-all ${playing ? "bg-white/10 shadow-none" : "bg-gradient-to-br from-blue-600 to-cyan-500 shadow-blue-500/30"}`}
      >
        {loading ? (
          ...
          {TRENDING.map((item, i) => {
            ...
            return (
              <motion.div
                ...
                className="w-44 shrink-0 cursor-pointer overflow-hidden rounded-2xl border"
                style={{ borderColor: "var(--app-border)" }}
              >
                {/* Thumbnail */}
                <div
                  className={`relative h-24 bg-gradient-to-br ${item.grad} flex items-center justify-center`}
                >
                  ...
                  <span
                    className={`absolute top-2 left-2 rounded-full border px-2 py-0.5 text-[9px] font-black ${TYPE_BADGE[item.type] ?? ""}`}
                  >
                    {item.type}
                  </span>
                  <span className="absolute right-2 bottom-1.5 text-[10px] font-bold text-white/80">
                    {item.dur}
                  </span>
                </div>
                {/* Info */}
        ...
        <div className="space-y-2">
          {RECENT.map((item, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`h-10 w-10 shrink-0 rounded-xl bg-gradient-to-br ${item.grad} flex items-center justify-center shadow`}
                >
                ...
                <span
                  className={`shrink-0 rounded-full border px-2 py-0.5 text-[9px] font-black ${TYPE_BADGE[item.type] ?? ""}`}
                >
                  {item.type}
                </span>
                <Play
